...
/-!
# Source-profile bridge for the semicircular transition cell

This module composes the raw-coordinate type-(iv) normalization and minimizer
quantifier transfer with the branch-complete transition exclusion at
`lambda = 5 / 4` and weighted area `9 * pi / 2`.  The transition theorem itself
classifies every regular equal-area root into its exact dyadic slab.
-/

noncomputable section

namespace TransitionSourceBridge
...
with the semicircular transition area can minimize. -/
theorem sourceCarrier_not_isMinimizer
    ...
    (harea :
      _root_.WeightedArea (5 / 4 : ℝ) sourceCarrier = 9 * Real.pi / 2) :
  ...
  have hcandidateArea :
      profile.toCandidate.WeightedArea = 9 * Real.pi / 2 := by
    calc
      ...
      _ = 9 * Real.pi / 2 := harea
  exact (TransitionCell.candidate_not_isWeightedPerimeterMinimizer
    profile.toCandidate
...
raw `E₄` carrier with the transition area is unconditionally non-minimizing. -/
theorem canonicalCarrier_not_isMinimizer
    (profile : CanonicalTypeIVProfile (5 / 4 : ℝ))
    (harea :
      _root_.WeightedArea (5 / 4 : ℝ) profile.carrier = 9 * Real.pi / 2) :
  ...
  have hcandidateArea :
      profile.toCandidate.WeightedArea = 9 * Real.pi / 2 :=
...
/-- Every horizontal translate of a raw `E₄` carrier with the transition area
is unconditionally non-minimizing in canonical complete-frontier semantics. -/
